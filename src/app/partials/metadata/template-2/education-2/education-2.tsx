...
import { EducationItem2 } from './education-2.items'
...
export const Education2 = forwardRef((props: EducationProps, ref) => {
  ...
  return (
    <div>
      {metaDataList.map((item, index) => (
        <EducationItem2 ref={(ref) => (item.ref = ref)} key={`education_${index}`} onBlur={onBlur} onFocus={onFocus} />
      ))}
    </div>
  )
})
