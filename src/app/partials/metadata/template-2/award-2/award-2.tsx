...
import { AwardItem2 } from './award-2.items'
...
export const Award2 = forwardRef((props: AwardProps, ref) => {
  ...
  return (
    <div>
      {metaDataList.map((item, index) => (
        <AwardItem2 ref={(ref) => (item.ref = ref)} key={`award_${index}`} onFocus={onFocus} onBlur={onBlur} />
      ))}
    </div>
  )
})
